...
- `GEMINI.md`, `CLAUDE.md`, `*.mjs` scripts, `dashboard/*`, `templates/*`, `batch/*`
...
AI-powered job search automation: pipeline tracking, offer evaluation, CV generation, portal scanning, batch processing. Compatible with Claude Code, Gemini CLI, OpenCode, and Codex.
...
**Note:** Gemini CLI commands route into the same `modes/*` files used by Claude Code and OpenCode. The mode files are shared across all platforms.

### Other Platform Commands

- **Claude Code:** `/career-ops {mode}` — see `CLAUDE.md`
- **OpenCode:** `/career-ops-{mode}` — see CLAUDE.md § OpenCode Commands
- **Codex:** Natural language prompts — see `docs/CODEX.md`
...
**NEVER trust a simple web fetch to verify if an offer is still active.** ALWAYS use browser tools when available:
1. Navigate to the URL
2. Read the page content
3. Only footer/navbar without JD = closed. Title + description + Apply = active.

**Exception for batch workers (headless mode):** Browser tools may not be available in headless/pipe mode. Use web fetch as fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.
